...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _sendChatMessage(String message) async {
    ...
    try {
      // Añadir un mensaje de carga mientras llega la respuesta
      _messages
          .add(ChatMessage(text: '...', isUser: false, time: formattedTime));

      // Detectar el idioma del mensaje (simple, basado en caracteres)
      bool isEnglish = RegExp(r'[a-zA-Z]').hasMatch(message);

      // Preparar la solicitud para el modelo
      final prompt = isEnglish
          ? message // Si es en inglés, enviar el mensaje tal cual
          : "Por favor, responde brevemente: $message"; // Si no, enviar con instrucción
      ...
      String text = response.text ?? 'No se recibió respuesta';
      text = text.length > 200 ? '${text.substring(0, 200)}...' : text;

      // Actualizar la lista de mensajes
      setState(() {
        _messages.removeLast(); // Eliminar el mensaje de carga
        _messages
            .add(ChatMessage(text: text, isUser: false, time: formattedTime));
      });

      // Desplazar hacia abajo, leer la respuesta y guardar el historial
      _scrollDown();
      await _speak(text);
      _saveChatHistory();
    } catch (e) {
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title: Text(
                    message.text,
                    style: TextStyle(
                        color: message.isUser ? const Color.fromARGB(255, 23, 23, 23) : Colors.black),
                  ),
                  subtitle:
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          if (!message.isUser)
            const CircleAvatar(
              backgroundImage: AssetImage('assets/UP.jpg'), // Imagen del bot
            ),
          Flexible(
            child: Padding(
          ...
          if (message.isUser)
            const CircleAvatar(
              backgroundImage: AssetImage('assets/imagen.jpeg'), // Imagen predeterminada
            ),
        ],
      ),
    );
